...
def knapsack_recursive(n, w, weights, values):
	...
	if weights[n-1] <= w:
		return max(values[n-1] + knapsack_recursive(n, w-weights[n-1], weights, values),
...
def knapsack(weights, values, C):
	N = len(weights)
	mem = [[-1 for c in range(N+1)] for r in range(C+1)]
	def knapsack_memoized(n, w, weights, values):
		...
		if weights[n-1] <= capacity:
			mem[n][w] = max(values[n-1] + knapsack_memoized(n, w-weights[n-1], weights, values),
	...
	knapsack_memoized(N, C, weights, values)
	...
	for n in range(N+1):
		for w in range(W+1):
			if n == 0 || w == 0:
				...
				dp[n][w] = max(dp[n-1][w], dp[n][w-weights[n]] + values[n-1])
			else:
				dp[n][w] = dp[n-1][w]
